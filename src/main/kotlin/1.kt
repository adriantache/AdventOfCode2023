...
fun main() {
    ...
    input.forEach { rowInput: String ->
        val row = rowInput.processNumbers(true)
        val smartRow = rowInput.processNumbers(false)

        val number = getNumber(row)
        val smartNumber = getNumber(smartRow)

        if(number != smartNumber) {
            if(row != smartRow) println("$rowInput -> $number != $smartNumber")
        }
...
private fun getNumber(row: String): Int {
    var indexForward = 0

    while (!row[indexForward].isDigit()) indexForward++
    val firstDigit = row[indexForward]

    var indexBackward = row.length - 1

    while (!row[indexBackward].isDigit() && indexBackward != indexForward) indexBackward--
    val secondDigit = row[indexBackward]

    return (firstDigit.toString() + secondDigit).toIntOrNull() ?: 0
}

private fun String.processNumbers(dumb: Boolean): String {
    var finalString = ""
    var index = 0

    while (index < this.length) {
        val currentLetter = this[index]

        if (currentLetter.isDigit() || currentLetter !in startingLetters) {
            finalString += currentLetter
            index++
            continue
        }

        val nextLetters = this.substring(index, this.length)
        var skip = false
        NUMBERS.forEach {
            if (skip) return@forEach

            if (nextLetters.startsWith(it.key)) {
                finalString += it.value
                if (!dumb) {
                    index += it.key.length
                } else {
                    index++
...
private val startingLetters = NUMBERS.keys.map { it.first() }.distinct()
