...
fun main() {
    ...
    val validGames = games.filter {
        it.maxRed <= expectedRed && it.maxGreen <= expectedGreen && it.maxBlue <= expectedBlue
    }

    println(validGames.joinToString("\n"))

    val result = validGames.sumOf { it.id }
    println(result)

    val result2 = games.sumOf { it.power }
    println(result2)
}
...
data class Game(
    ...
    val maxRed = steps.maxOf { it.red }
    val maxGreen = steps.maxOf { it.green }
    val maxBlue = steps.maxOf { it.blue }
    val power = maxRed * maxGreen * maxBlue

    companion object {
        fun fromString(string: String): Game {
            val firstSplit = string.split(": ")
            val id = firstSplit[0].split(" ")[1].toInt()

            val stepStrings = firstSplit[1].split("; ")
            val steps = stepStrings.map { GameStep.fromString(it) }

            return Game(id, steps)
        }
    }
}
...
data class GameStep(
    ...
    companion object {
        fun fromString(string: String): GameStep {
            val components = string.split(", ").associate {
                val component = it.split(" ")
                component[1] to component[0]
            }

            return GameStep(
                red = components["red"]?.toInt() ?: 0,
                green = components["green"]?.toInt() ?: 0,
                blue = components["blue"]?.toInt() ?: 0,
            )
        }
    }
}
