...
fun main() {
    val input = INPUT.split("\n").filterNot { it.isBlank() }.toTypedArray()

    val okNumbers = mutableListOf<String>()
    ...
    input.forEachIndexed { j, row ->
        ...
        for (i in row.indices) {
            ...
            if (character.isDigit()) {
                ...
                if (i == row.length - 1) {
                    val number = row.substring(startI..i)
                ...
                if (startI != null) {
                    ...
                    val number = row.substring(startI..endI)
                    val isValid = validateNumber(
                        numberMinI = startI,
                        numberMaxI = endI,
                        numberJ = j,
    ...
    println("Engine parts: ${okNumbers.sumOf { it.toInt() }}")

    findGears(input)
}

private fun findGears(input: Array<String>) {
    val gearRatios = mutableListOf<Long>()

    input.forEachIndexed { j, row ->
        row.forEachIndexed loop@{ i, char ->
            // 1. Find gear
            if (char == '*') {
                // 2. Count numbers around, if != 2 move on
                val numbers = findNumbers(i, j, input)

                if (numbers.size != 2) return@loop
                // 3. Resolve rest of the numbers
                val fullNumbers = numbers.map { it.findFullNumber(input) }

                val gearRatio = fullNumbers.fold(1L) { acc, number -> acc * number }
                gearRatios += gearRatio
            }
        }
    }

    println("Gear ratios: ${gearRatios.sum()}")
}

private fun NumberCoords.findFullNumber(input: Array<String>): Long {
    val row = input[this.j]

    var startI = this.i

    for (i in (this.i-1).coerceAtLeast(0) downTo 0) {
        if (row[i].isDigit()) {
            startI = i
        } else {
            break
    ...
    for (i in (this.i + 1).coerceAtMost(row.length - 1) until row.length) {
        if (row[i].isDigit()) {
            endI = i
        } else {
            break
    ...
    val number = row.substring(startI, endI + 1)
...
private fun findNumbers(
    ...
    val minI = (gearI - 1).coerceAtLeast(0)
    val maxI = (gearI + 1).coerceAtMost(input[gearJ].length - 1)

    val minJ = (gearJ - 1).coerceAtLeast(0)
    val maxJ = (gearJ + 1).coerceAtMost(input.size - 1)

    val numbers = mutableListOf<NumberCoords>()

    for (j in minJ..maxJ) {
        for (i in minI..maxI) {
            if (i == gearI && j == gearJ) continue

            val char = input[j][i]

            if (char.isDigit()) {
                numbers += NumberCoords(i, j)
            }
        }
    }

    return numbers.filterNeighbours()
}

private fun List<NumberCoords>.filterNeighbours(): List<NumberCoords> {
    var last: NumberCoords? = null

    return this.mapNotNull {
        // Filter out consecutive numbers on the same row.
        if (last?.i == it.i - 1 && last?.j == it.j) {
            last = it
...
private data class NumberCoords(
    val i: Int,
    val j: Int,
)
...
private fun validateNumber(
    ...
    val minI = (numberMinI - 1).coerceAtLeast(0)
    val maxI = (numberMaxI + 1).coerceAtMost(input[numberJ].length - 1)

    val minJ = (numberJ - 1).coerceAtLeast(0)
    val maxJ = (numberJ + 1).coerceAtMost(input.size - 1)

    for (j in minJ..maxJ) {
        for (i in minI..maxI) {
            if (j == numberJ && i in numberMinI..numberMaxI) continue

            val symbol = input[j][i]
