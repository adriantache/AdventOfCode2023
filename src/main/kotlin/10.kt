...
fun main() {
    val input = INPUT.split("\n").filterNot { it.isBlank() }
        .map { row ->
            row.split("").filterNot { it.isEmpty() }
        }
...
private fun findFirstDirection(
    ...
    val maxX = (startX + 1).coerceAtMost(input[startY].size - 1)

    val minY = (startY - 1).coerceAtLeast(0)
    val maxY = (startY + 1).coerceAtMost(input.size - 1)
